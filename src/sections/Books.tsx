		...
		>
			...
			<div className="mt-24 relative z-10 max-w-7xl mx-auto px-6 md:px-12">
				...
				>
					...
					>
						<img
							src={books.galleryImages[0].src}
							alt={books.galleryImages[0].alt}
						...
						<div className="relative z-10">
							...
							<div className="grid items-start gap-6 lg:grid-cols-[190px_minmax(0,1fr)] lg:gap-8">
								<div className="overflow-hidden rounded-2xl border border-white/12 bg-white/6 shadow-[0_24px_60px_rgba(0,0,0,0.24)] backdrop-blur-sm max-w-[220px]">
									<img
										src={books.showcaseImages[0].src}
										alt={books.showcaseImages[0].alt}
										className="h-full w-full object-cover"
										loading="lazy"
									/>
								</div>
								<div>
									<p className="text-white text-xs font-body uppercase tracking-[0.28em] mb-4">
										{books.publishedBook.eyebrow}
									</p>
									<h3 className="max-w-2xl text-3xl md:text-4xl lg:text-5xl font-sans font-bold text-white tracking-tight leading-tight">
										{books.publishedBook.title}
									</h3>
                  <p className="mt-6 max-w-2xl text-white/65 font-body text-lg md:text-xl leading-relaxed">
                    {books.publishedBook.description}
                  </p>
								</div>
							</div>
						</div>
						...
						<div className="relative z-10 mt-10">
							<Button
								asChild
								size="lg"
								className="bg-secondary text-white hover:bg-white hover:text-primary rounded-full px-7 shadow-none"
							>
								<a href={AMAZON_URL} target="_blank" rel="noreferrer">
									{books.publishedBook.buttonLabel}
					...
					>
						<img
							src={books.galleryImages[1].src}
							alt={books.galleryImages[1].alt}
						...
						<div className="relative z-10">
							...
							<p className="text-white text-xs font-body uppercase tracking-[0.28em] mb-4">
								{books.upcomingBook.eyebrow}
							</p>
							<h3 className="max-w-xl text-3xl md:text-4xl font-sans font-bold text-white tracking-tight leading-tight">
								{books.upcomingBook.title}
							</h3>
              <p className="mt-6 max-w-xl text-white font-body text-lg md:text-xl leading-relaxed">
                {books.upcomingBook.description}
              </p>
						</div>
						...
						<div className="relative z-10 mt-10">
							...
							>
								<a href={EARLY_ACCESS_URL} target="_blank" rel="noreferrer">
									{books.upcomingBook.buttonLabel}
									<ArrowUpRight className="h-4 w-4" />
								</a>
							</Button>
						</div>
